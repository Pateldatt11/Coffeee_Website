...
const Profile = () => {
    ...
    const unsubGlobal = onSnapshot(
      ...
      (docSnap) => {
        if (docSnap.exists() && docSnap.data().message) {
          setGlobalAdminMsg(docSnap.data().message);
        }
      },
      () => {},
    );
  ...
  const displayName =
    profile.name || user?.displayName || user?.email?.split("@")[0] || "Brewer";
  const isAdmin = profile.role === "admin" || user?.email?.includes("admin");
  const activeAdminMessage = profile.adminMessage || globalAdminMsg;
  ...
  const nextTierAmount =
    TOKEN_TIER_STEPS.find((t) => rewards.tokens < t.amount)?.amount ??
    TOKEN_TIER_STEPS[TOKEN_TIER_STEPS.length - 1].amount;
  const progressPercent = Math.min(
    100,
    Math.round((rewards.tokens / nextTierAmount) * 100),
  );
  ...
  const submitFeedback = async () => {
    ...
    try {
      ...
      setOrders((prev) =>
        prev.map((o) =>
          o.id === feedbackOrder.id ? { ...o, feedbackGiven: true } : o,
        ),
      );
    ...
    <div className="profile-page">
            ...
            <div className="token-tier-markers">
              {TOKEN_TIER_STEPS.map((tier) => (
                <span
                  key={tier.amount}
                  className={
                    rewards.tokens >= tier.amount ? "tier-reached" : ""
                  }
                >
            ...
            {!showReferral ? (
              ...
              <>
                ...
                <p className="referral-hint">
                  Share this link — you earn ₹50 for every friend who signs up
                  using it.
                </p>
              </>
            )}
                  ...
                  >
                          ...
                          >
                            <img
                              src={
                                ...
                                it.image ||
                                it.photoURL ||
                                "/coffee-placeholder.png"
                              }
                              alt={it.name || "Coffee item"}
                              className="order-line-img"
                              onError={(e) => {
                                e.currentTarget.src = "/coffee-placeholder.png";
                              }}
                            />
                    ...
                    {(order.walletUsed > 0 ||
                      ...
                      <div className="order-rewards-line">
                        ...
                        {order.tokenDiscount > 0 && (
                          <span>Token Discount: −₹{order.tokenDiscount}</span>
                        )}
                        {order.walletUsed > 0 && (
                          <span>Wallet Used: −₹{order.walletUsed}</span>
                        )}
                        {order.tokensEarned > 0 && (
                          <span>🪙 Earned: +{order.tokensEarned}</span>
                        )}
                      </div>
                    )}
                    ...
                    >
                      <span className={`pay-badge ${meta.className}`}>
                        {meta.label}
                      </span>
                    ...
                    {status === "completed" && order.feedbackGiven && (
                      <p className="order-feedback-done">
                        ✓ Thanks — you've rated this order
                      </p>
                    )}
                  </li>
                );
      ...
      {feedbackOrder && (
        <div className="details-overlay" onClick={closeFeedback}>
          <div
            className="details-modal feedback-modal"
            onClick={(e) => e.stopPropagation()}
          >
            <button className="details-close" onClick={closeFeedback}>
              ✕
            </button>
            ...
            {feedbackSubmitted ? (
              ...
              <div className="details-body">
                <p className="category-tag">
                  Order #{feedbackOrder.id.slice(-6).toUpperCase()}
                </p>
